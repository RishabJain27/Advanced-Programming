...
//this is a map of all the fonts available
...
//this is a map of the different shapes that are going to be made
...
shape_ptr interpreter::make_circle (param begin, param end) {
  DEBUGF ('f', range (begin, end));
  if (begin == end)
    throw runtime_error("Circle defined without diameter.");
  if (end - begin != 1)
...
shape_ptr interpreter::make_square (param begin, param end) {
  DEBUGF ('f', range (begin, end));
  if (begin == end)
    throw runtime_error("square does not have side length");
  if (end - begin != 1) 
    throw runtime_error ("square: only one length needed");
  auto iter = begin;
